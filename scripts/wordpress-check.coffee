  ...
  parser url, (err,rss)->
    ...
    if pubDate.getMonth() == today.getMonth() && pubDate.getDate() == today.getDate() && pubDate.getHours() == today.getHours()
      if 'Security' in rss[0].categories
        # console.log "#{colors.bold.red('URGENT!')} #{rss[0].title} - #{rss[0].link}\nPublished: #{rss[0].date}"
        return "#{colors.bold.red('URGENT!')} #{rss[0].title} - #{rss[0].link}\nPublished: #{rss[0].date}"
      else
        # console.log "Sweet #{rss[0].categories.join(" ")} available! #{rss[0].title} - #{rss[0].link}\nPublished: #{rss[0].date}"
        return "Sweet #{rss[0].categories.join(" ")} available! #{rss[0].title} - #{rss[0].link}\nPublished: #{rss[0].date}"
    # else
    #   console.log colors.bold.red('URGENT!')
